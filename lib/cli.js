...
function logResult(result, finalResult)
{
	...
	if (result.__cli_excluded !== true)
	{
		...
		output = chalk.gray( finalResult!==true ? "--" : "__" );
		...
		else if (result.url.redirected != null) {
			output += chalk.gray("--");
			output += chalk.red("REDIRECTED");
			output += chalk.gray("-- ");
		}
		else
		{
		...
		if (result.url.redirected != null) {
			output += chalk.gray(" => ");
			output += chalk.yellow( result.url.redirected );
		}
